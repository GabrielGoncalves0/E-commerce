...
class ProdutoModel {
    ...
    async excluir(codigo){
        ...
        let sqlProdutoItens = "delete from tb_pedidoitens where prd_id = ?;";
        let valores = [codigo];

        await conexao.ExecutaComandoNonQuery(sqlProdutoItens, valores);
    ...
    async gravar() {
        if(this.#produtoId == 0){
            ...
            let valores = [this.#produtoCodigo, this.#produtoNome, this.#produtoQuantidade, this.#categoriaId, 
    ...
    async atualizarEstoque() {
            ...
            let sql = "update tb_produto set prd_quantidade= ?, prd_valor = ? where prd_id = ?";

            let valores = [this.#produtoQuantidade, this.#produtoValor, this.#produtoId];
